...
bool ShouldReport(ThreadState *thr, ReportType typ) {
  Printf("bool ShouldReport(ThreadState *thr, ReportType typ)");
...
int ScopedReportBase::AddMutex(uptr addr, StackID creation_stack_id) {
  Printf("ScopedReportBase::AddMutex\n"); 
...
void ScopedReportBase::AddLocation(uptr addr, uptr size) {
  Printf("ScopedReportBase::AddLocation\n"); 
  if (addr == 0)
    return;
#if !SANITIZER_GO
  int fd = -1;
...
ALWAYS_INLINE USED void PrintCurrentStackSlow(uptr pc) {
  Printf("void PrintCurrentStackSlow\n");
#if !SANITIZER_GO
