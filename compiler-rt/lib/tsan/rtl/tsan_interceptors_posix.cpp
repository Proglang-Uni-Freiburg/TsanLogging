...
TSAN_INTERCEPTOR(int, pthread_join, void *th, void **ret) {
  ...
  //__tsan::tsanInterceptorsAndMemoryAccessOperationsLogging("pthread_join is invoked", (void*)th, thr->tid); 
  ...
  ThreadIgnoreEnd(thr);
  if (res == 0) {
   // __tsan::tsanInterceptorsAndMemoryAccessOperationsLogging("Join at ", (void*)th, thr->tid);
    __tsan::tsanInterceptorsAndMemoryAccessOperationsLogging("|Join", (void*)th, thr, 0, tid);
...
TSAN_INTERCEPTOR(int, pthread_mutex_unlock, void *m) {
  ...
  //__tsan::tsanInterceptorsAndMemoryAccessOperationsLogging("pthread_mutex_unlock is invoked", (void*)m, thr);
  MutexUnlock(thr, pc, (uptr)m);
...
TSAN_INTERCEPTOR(int, fork, int fake) {
  ...
  Printf("TSAN_INTERCEPTOR fork is called %d and fake %d\n", fork, fake);
...
void atfork_prepare() {
  if (in_symbolizer())
    return;
  ThreadState *thr = cur_thread();
  int cnt = ThreadCount(thr);
 // Printf("atfork_prepare is called at trace pos %d \n", thr->trace_pos);
  Printf("At fork Prepare Thread Count: %d\n", cnt);
...
void atfork_parent() {
  ...
  const uptr pc = StackTrace::GetCurrentPc();
  //int cnt = ThreadCount(thr);
  //Tid id = ThreadConsumeTid(thr, pc, mtx);
  //Printf("Parent fork at trace pos %d\n",thr->trace_pos);
  Printf("Before Parent fork\n");
...
void atfork_child() {
  ...
  ThreadState *thr = cur_thread();
  const uptr pc = StackTrace::GetCurrentPc();
  //Printf("Child fork %d\n",ThreadConsumeTid(thr, pc, thr->tid));
  //Printf("Child fork %d\n",);
  //int cnt = ThreadCount(thr);
  //Printf("Child fork at trace pos %d\n",thr->trace_pos);
  Printf("Child Thread fork \n");
...
TSAN_INTERCEPTOR(int, clone, int (*fn)(void *), void *stack, int flags,
  ...
  auto wrapper = +[](void *p) -> int {
    ...
    Printf("Clone interceptor called\n");
...
static int OnExit(ThreadState *thr) {
  //Printf("ThreadSanitizer: thread %d finished OnExit\n", thr->tid);
...
static void finalize(void *arg) {
  Printf("\nThreadSanitizer: thread %d finished finalize\n", cur_thread()->tid);
  ThreadState *thr = cur_thread();
  int status = Finalize(thr);
