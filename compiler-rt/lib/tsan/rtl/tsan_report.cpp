...
static void PrintMop(const ReportMop *mop, bool first) {
  ...
  Printf(":\n");
  Printf("%s", d.Default());
  //Printf("Before PrintStack in PrintMop\n");
...
static void PrintLocation(const ReportLocation *loc) {
  ...
  Printf("%s", d.Default());
  if (print_stack)
    //Printf("Before PrintStack in PrintLocation\n");
    PrintStack(loc->stack);
  //Printf("End PrintLocation\n");
}
...
static ReportStack *ChooseSummaryStack(const ReportDesc *rep) {
  Printf("ReportStack *ChooseSummaryStack\n");
  if (rep->mops.Size())
...
void PrintReport(const ReportDesc *rep) {
  ...
  if (rep->typ == ReportTypeDeadlock) {
    ...
    for (uptr i = 0; i < rep->stacks.Size(); i++) {
      if (i)
        Printf("  and:\n");
      //Printf("Before PrintStack(rep->stacks[i])\n");
      PrintStack(rep->stacks[i]);
    }
  }
