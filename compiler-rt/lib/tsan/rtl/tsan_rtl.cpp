...
int Finalize(ThreadState *thr) {
  ...
  // Custom log message
  Printf("Finalize function started\n");
  //printf("Finalize function started\n");
  ...
  if (ctx->nreported) {
    failed = true;
#if !SANITIZER_GO
    //fprintf(stderr, "ThreadSanitizer: after warnings are reported\n");
    //fflush(stderr);
    //Printf("ThreadSanitizer: after warnings are reported");
    //std::cout << "More warnings messages" << std::endl;
    //DPrintf("Dummy log message");
    //printf("Some warning messages");
    //VPrintf("VPrintf messages");
    Printf("ThreadSanitizer: reported2 %d warnings\n", ctx->nreported);
    #ifdef SANITIZER_DEBUG
       Printf("TSAN Debug: Custom log message here!\n");
    #endif
...
// #if !SANITIZER_GO
// size_t messageLength(const char* message)
// {
//   size_t length = 0;
...
// void hexadecimalString(unsigned long long value, char *buf)
// {
...
// void intToDecStr(unsigned long long value, char *buf)
// {
//   int i=0;
...
// void instrumentationHookLogMessage(const char* logMessage, void *addr, Tid tid, uptr callerpc)
// {
...
//   if (!logClear)
//   {
//     instrumentationHookLogFlag = open("instrumentationHookLog.txt", O_WRONLY | O_CREAT | O_TRUNC, 0644);
//     logClear = true;
//   }
//   else
//   {
//     instrumentationHookLogFlag = open("instrumentationHookLog.txt", O_WRONLY | O_CREAT | O_APPEND, 0644);
//   }
//   if (instrumentationHookLogFlag != -1)
//   {

//     char threadIdBuf[30];
//     intToDecStr((unsigned long long)tid, threadIdBuf);
//     write(instrumentationHookLogFlag, "Thread ", 7);
//     write(instrumentationHookLogFlag, threadIdBuf, messageLength(threadIdBuf));
//     write(instrumentationHookLogFlag, ": ", 2);

//     write(instrumentationHookLogFlag, logMessage, messageLength(logMessage));

//     char addrBuf[18] = " at 0x";
//     hexadecimalString((unsigned long long)addr, addrBuf + 6);
//     write(instrumentationHookLogFlag, addrBuf, messageLength(addrBuf));
...
//         char lineBuf1[20];
//         intToDecStr(info.line, lineBuf1);
//         write(instrumentationHookLogFlag, " at line ", 9);
//         write(instrumentationHookLogFlag, lineBuf1, messageLength(lineBuf1));

//         write(instrumentationHookLogFlag, " in file ", 9);
//         write(instrumentationHookLogFlag, info.file, messageLength(info.file));
...
//     //   DataInfo info;
//     //   bool var = Symbolizer::GetOrInit()->SymbolizeData((uptr)addr, &info);
//     //   if(var)
//     //   {
//     //     Printf("SymbolizedStack is not null\n");
//     //     Printf("The line number is %d", info.line);
        
//     //     //char lineBuf[20];
//     //     // write(instrumentationHookLogFlag, );
//     //     // write(instrumentationHookLogFlag, info.function, messageLength(info.function));

//     //     //if(info.line != 0)
//     //     //{
//     //      // Printf("Line number is not 0\n");
        
//     //       // write(instrumentationHookLogFlag, " in ", 4);
//     //       // write(instrumentationHookLogFlag, (const char*)info.file, messageLength(info.file));
//     //       // write(instrumentationHookLogFlag, ":", 1);

//     //       // char addrBuf1[20] = " at addr";
//     //       // hexadecimalString(info.address, addrBuf1 + 8);
//     //       // write(instrumentationHookLogFlag, addrBuf1, messageLength(addrBuf1));
//     //       char lineBuf1[20];
//     //       intToDecStr(info.line, lineBuf1);
//     //       write(instrumentationHookLogFlag, " at line ", 9);
//     //       write(instrumentationHookLogFlag, lineBuf1, messageLength(lineBuf1));
...
// //-----------------------------------------------------------------------------------------------------
//     write(instrumentationHookLogFlag, "\n", 1);
//     close(instrumentationHookLogFlag);
//   }
//   else
//   {
